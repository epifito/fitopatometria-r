...
# Dataset containing simulated data of mycelial growth under increasing fungicide doses 
# for: 
...
# importante chequear el data type ya que ajustaremos regresiones, por lo tanto necesitamos 
...
model.LL3 <- drm(growth ~ dose, data = iso1_org_A, 
                 fct = LL.3(names = c("Slope", "Upper Limit", "ED50")))
plot(model.LL3)

model.LL4 <- drm(growth ~ dose, data = iso1_org_A, 
...
# avanzamos con LL.3
summary(model.LL3)
ED(model.LL3, respLev = c(50), interval="delta", type = "relative")
